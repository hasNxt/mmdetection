...
"""
Dataset used with HPN.
Needs 2 datasets to be present:
    1) The dataset in coco format, where everything is class person
    2) A second dataset with the 'posed_<dataset name>' 
    with the same annotations of the first one, but the correct classes for each orientation
"""
import contextlib
import io
import itertools
...
class TC8CocoDataset(CustomDataset):
    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            #     (pose_acc, pose_acc_noBBox, classwise_acc) = self.get_accuracy(results)
