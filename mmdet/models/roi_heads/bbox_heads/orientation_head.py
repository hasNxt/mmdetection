...
class SigmaOrientationHead(BBoxHead):
    ...
    def __init__(self,
        ...
        if self.with_cls:
            ...
            # self.fc_cls._parameters['log_var_cls'] = torch.zeros((1,), requires_grad=True)
        ...
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                ...
                if isinstance(loss_cls_, dict):
                    ...
                    # self.log_var_cls = self.log_var_cls.to(loss_cls_.device)
                    # precision = torch.exp(-self.log_var_cls).to(loss_cls_.device)
                    # weight_loss = torch.sum(precision*loss_cls_ + self.log_var_cls, -1)
                    ...
                    losses['loss_pose'] = weight_loss
                    losses['log_var_hpn'] = self.fc_cls.get_parameter('log_var_cls')
                    losses['hpn_precision'] = precision
                    # losses['loss_pose'] = loss_cls_
